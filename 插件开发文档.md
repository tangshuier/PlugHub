...
WPF插件工具箱是一个基于WPF和MVVM框架的插件管理系统，支持热插拔插件、插件依赖管理、插件UI窗口和调试功能。本文档将指导开发者如何开发符合规范的插件。
...
2. 添加以下NuGet包：
   - `System.ComponentModel.Composition` (MEF支持)
3. 添加对WPF工具箱核心API的引用：
...
创建一个实现 `IPlugin` 或 `IDependency` 接口的类，并使用MEF导出：
...
[Export(typeof(IPlugin))] // 必须使用MEF导出
...
| 问题 | 可能原因 | 解决方案 |
|------|----------|----------|
| 插件无法加载 | - 缺少MEF导出属性<br>- 依赖缺失<br>- 代码错误 | - 检查是否添加了`[Export]`属性<br>- 检查依赖是否存在<br>- 查看调试窗口的错误信息 |
...
| 依赖无法获取 | - 依赖ID错误<br>- 依赖未正确导出 | - 检查依赖ID是否正确<br>- 确保依赖添加了`[Export]`属性 |
